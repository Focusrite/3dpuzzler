...
package body Algorithm is
   ...
   begin
	 ...
	 Put("Using Part ID: "); Put(Get_Id(Current_Part), 0); Put(", p� plats: ");Put("X: "); Put(Get_X(Current_Part), 0); Put(", Y: "); Put(Get_Y(Current_Part), 0); Put(", Z: "); Put(Get_Z(Current_Part), 0); Put(", R: "); Put(Get_Rotation(Current_Part), 0); Put(", pWidth: "); Put(Get_Width(Current_Part), 0); Put(", gWidth: "); Put(Get_Width(Goal), 0);New_Line;
	 if Get_Z(Current_Part) + Get_Depth(Current_Part) - 1 > Get_Depth(Goal) and not Rotations_Left then
	    ...
	    Put("X: "); Put(Get_X(Current_Part), 0); Put(", Y: "); Put(Get_Y(Current_Part), 0); Put(", Z: "); Put(Get_Z(Current_Part), 0);New_Line;
	    Put_Line("Part: ");
	    Put(Current_Part);
	    Put_Line("Goal: ");
	    Put(Goal);
	    ...
	    if Get_Y(Current_Part) + Get_Height(Current_Part) - 1 > Get_Height(Goal) and not Rotations_Left Then
		  ...
		  if not Backed and Fits(Current_Part.all, Goal.all) then
			...
			--Put("Roterar till: "); Put(Get_Rotation(Current_Part), 0);New_Line;
		     end if;
		  end if;
	       end if;
	    end if;
