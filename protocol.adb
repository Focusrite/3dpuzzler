...
package body Protocol is
   ...
   begin
	 ...
	 if Get_Id(Current_Part) = Cur_Id then
	    Resultstr := Resultstr & " !"
	      ...
	      & Integer'Image(Get_Height(Figure)- Get_Height(Current_Part) - Get_Y(Current_Part))
	      & Integer'Image(Get_Z(Current_Part));
	    Cur_Id := Cur_Id + 1;
	 end if;
	 Put(Length(Solved_Figure_List), 0);
	 exit when Length(Solved_Figure_List) < Cur_Id;
	 --exit when Last_Element(Solved_Figure_List) = Current_Part.all;
	 if Current_Part.all = Last_Element(Solved_Figure_List) then
	    First(Solved_Figure_List, Current_Part);
	 else
