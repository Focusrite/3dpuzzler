...
package body Protocol is
   ...
   begin
	  ...
	  Msg.Header  := Slice(Msg_S, 9, 10)(1);
	  Msg.Message := To_Unbounded_String(Slice(Msg_S, 11, Length(Msg_S)));
	  return Msg;
   end To_Message;
   ...
   function Solution_Message_Str(Solved_Figure_List: in out Figure_List_Type) return Unbounded_String is
	Resultstr: Unbounded_String;
	Current_Part: Figure_Access := First(Solved_Figure_List);
   begin
	loop --'image sätter mellanslag innan varje
		...
		Current_Part:=Next(Solved_Figure_List);
	exit when Last_Element(Solved_Figure_List) = Current_Part.all;
	end loop;
	return Resultstr;
   end Solution_Message_Str;
   
   procedure Send_Solution(Socket: in Socket_Type; Part_List: in out Figure_List_Type) is
	Solution_Message: Message_Type;
   begin
	Solution_Message.Header := 'S';
	Solution_Message.Message := Solution_Message_Str(Part_List);
   ...
   function Receive_Figure(Socket: in Socket_Type) return Figure_Type is
...
--	Dim_String := To_Unbounded_String(Slice(Msg, Start_Dim_Index, Start_Shape_Index - 2)); ta bort
	...
	return Create_Figure(Dim_X, Dim_Y, Dim_Z, Shape_String, Figure_Number); --Dummy-funktion! Se över detta med de andra!
   end Receive_Figure;
   
   
--TODO:          ------------------------------------------------------------------SKAPA SHAPEMATRIX AV STRING
   ...
   begin
	...
	for I in 1..Number_Of_Parts loop
		...
		Append(Part_List, Create_Figure(Dim_X, Dim_Y, Dim_Z, Shape_String)); --DUMMY-FUNKTION, inkludera nummer-integer också
  ...
  begin
   ...
   begin
	...
	return Integer'Value(Slice(Msg, 1, Index));
   end Extract_Number;
