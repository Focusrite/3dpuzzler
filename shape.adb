...
package body Shape is
   ...
   function Standardize(Shape : Shape_Matrix; Size, Offset : Axis_Vector) 
		       return Shape_Matrix is
      New_Shape : Shape_Matrix(Size(AXIS_X), Size(AXIS_Y), Size(AXIS_Z));
   begin
      for Z in New_Shape'Range(3) loop
	    for Y in New_Shape'Range(2) loop
		  for X in New_Shape'Range(1) loop
		     if (X - Offset(AXIS_X)) in New_Shape'Range(1) and (Y - Offset(AXIS_Y)) in New_Shape'Range(2) (Z - Offset(AXIS_Z)) in New_Shape'Range(3)  then
   ...
   begin
      ...
      if Axis = AXIS_X then
	 return Shape'Range(1);
      elsif Axis = AXIS_Y then
	 return Shape'Range(2);
      elsif Axis = AXIS_Z then
	 return Shape'Range(3);
      end if;
      return 1..1;
   end Size;
end Shape;
